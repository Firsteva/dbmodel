...
insert into MEMBERS(UID,PWD,EMAIL,UNAME,TEL)
values('u10','1111','u10@test.com','이봉찬','111-1111');

/* 문제: 회원의 아이디와 이름, 이메일을 출력하라. */
select UID,UNAME,EMAIL from MEMBERS;

/* 문제: 위의 결과를 출력할 때 아이디와 이름의 라벨명을 id, name으로 바꿔라.
 * 컬럼명 as 별명
 * 컬럼명 별명 <= as 생략 가능
 * 컬럼명 (as) 별명
 */
select UID as ID,UNAME NAME, EMAIL from MEMBERS;

/* 문제: 회원 정보를 다음과 같이 출력하시오.
 * 회원명(이메일)
 */
select concat(UNAME,'(',EMAIL,')') as Username_Email from MEMBERS;
...
/* UPDATE 명령
문법 : update 테이블명 set 컬럼명=값,컬럼명=값 where PK컬럼=값;
*/
